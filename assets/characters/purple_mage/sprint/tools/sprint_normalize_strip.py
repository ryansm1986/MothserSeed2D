...
def normalize_strip(source: Path, direction: str, sprint_dir: Path) -> dict[str, str]:
    raw_dir = sprint_dir / "raw"
    frames_dir = sprint_dir / "frames"
    preview_dir = sprint_dir / "preview"
    gif_dir = sprint_dir / "gif"
    for folder in (raw_dir, frames_dir, preview_dir, gif_dir):
        folder.mkdir(parents=True, exist_ok=True)

    source_strip = raw_dir / f"sprint_{direction}_8frame_staff_right_hand_green_strip.png"
    source_original = raw_dir / f"sprint_{direction}_8frame_staff_right_hand_green_strip_original.png"
    ...
    alpha_strip = raw_dir / f"sprint_{direction}_8frame_staff_right_hand_alpha_strip.png"
    raw_alpha = raw_dir / f"sprint_{direction}_raw.png"
    ...
    for idx, crop in enumerate(crops, 1):
        ...
        frame_path = frames_dir / f"sprint_{direction}_{idx:02d}.png"
        frame.save(frame_path)
        frames.append(frame)

    normalized_strip = raw_dir / f"sprint_{direction}_normalized_strip.png"
    ...
    preview_path = preview_dir / f"sprint_{direction}_preview.png"
    preview_4x_path = preview_dir / f"sprint_{direction}_preview_4x.png"
    ...
    green_preview_path = preview_dir / f"sprint_{direction}_green_preview_4x.png"
    green_preview.resize((green_preview.width * 4, green_preview.height * 4), Image.Resampling.NEAREST).save(
        green_preview_path
    )

    gif_path = gif_dir / f"sprint_{direction}.gif"
...
def write_qa(direction: str, sprint_dir: Path, base_sources: str) -> None:
    qa_dir = sprint_dir / "qa"
    qa_dir.mkdir(parents=True, exist_ok=True)
    path = qa_dir / f"sprint_{direction}_qa.md"
    path.write_text(
        f"""# sprint_{direction} QA

- Source strip: `assets/characters/purple_mage/sprint/raw/sprint_{direction}_8frame_staff_right_hand_green_strip.png`
- Alpha strip: `assets/characters/purple_mage/sprint/raw/sprint_{direction}_raw.png`
- Normalized strip: `assets/characters/purple_mage/sprint/raw/sprint_{direction}_normalized_strip.png`
- Normalized frames: `assets/characters/purple_mage/sprint/frames/sprint_{direction}_01.png` through `sprint_{direction}_08.png`
- Preview: `assets/characters/purple_mage/sprint/preview/sprint_{direction}_preview.png`
- 4x preview: `assets/characters/purple_mage/sprint/preview/sprint_{direction}_preview_4x.png`
- Green 4x preview: `assets/characters/purple_mage/sprint/preview/sprint_{direction}_green_preview_4x.png`
- GIF: `assets/characters/purple_mage/sprint/gif/sprint_{direction}.gif`
- Mapped base source(s): `{base_sources}`
- Local tool: `assets/characters/purple_mage/sprint/tools/sprint_normalize_strip.py`

Checks performed:

- Eight {direction}-facing sprint frames exported at 128x128 RGBA.
...
def main() -> None:
    ...
    parser.add_argument("--sprint-dir", default=Path(__file__).resolve().parents[1], type=Path)
    parser.add_argument("--base-sources", required=True)
    args = parser.parse_args()

    result = normalize_strip(args.source, args.direction, args.sprint_dir)
    write_qa(args.direction, args.sprint_dir, args.base_sources)
