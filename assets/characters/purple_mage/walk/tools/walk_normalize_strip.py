...
def normalize_strip(source: Path, direction: str, walk_dir: Path) -> dict[str, str]:
    raw_dir = walk_dir / "raw"
    frames_dir = walk_dir / "frames"
    preview_dir = walk_dir / "preview"
    gif_dir = walk_dir / "gif"
    for folder in (raw_dir, frames_dir, preview_dir, gif_dir):
        folder.mkdir(parents=True, exist_ok=True)

    green_original = raw_dir / f"walk_{direction}_6frame_staff_right_hand_green_strip_original.png"
    green_strip = raw_dir / f"walk_{direction}_6frame_staff_right_hand_green_strip.png"
    if source.resolve() != green_original.resolve():
        green_original.write_bytes(source.read_bytes())
    green_strip.write_bytes(source.read_bytes())

    img = Image.open(source).convert("RGBA")
    rgba, alpha, runs = make_alpha_source(img)
    alpha_strip = raw_dir / f"walk_{direction}_6frame_staff_right_hand_alpha_strip.png"
    raw_alpha = raw_dir / f"walk_{direction}_raw.png"
    ...
    for idx, crop in enumerate(crops, 1):
        ...
        frame_path = frames_dir / f"walk_{direction}_{idx:02d}.png"
        frame.save(frame_path)
        frames.append(frame)

    normalized_strip = raw_dir / f"walk_{direction}_normalized_strip.png"
    ...
    preview = Image.new("RGBA", (FRAME_SIZE * FRAME_COUNT + 8 * (FRAME_COUNT - 1), FRAME_SIZE), (60, 60, 60, 255))
    for idx, frame in enumerate(frames):
        preview.alpha_composite(frame, (idx * (FRAME_SIZE + 8), 0))
    preview_path = preview_dir / f"walk_{direction}_preview.png"
    preview_4x_path = preview_dir / f"walk_{direction}_preview_4x.png"
    ...
    green_preview_path = preview_dir / f"walk_{direction}_green_preview_4x.png"
    green_preview.resize((green_preview.width * 4, green_preview.height * 4), Image.Resampling.NEAREST).save(green_preview_path)

    gif_path = gif_dir / f"walk_{direction}.gif"
    frames[0].save(gif_path, save_all=True, append_images=frames[1:], duration=120, loop=0, disposal=2)
...
def write_qa(direction: str, walk_dir: Path, base_sources: str) -> None:
    qa_dir = walk_dir / "qa"
    qa_dir.mkdir(parents=True, exist_ok=True)
    path = qa_dir / f"walk_{direction}_qa.md"
    path.write_text(
        f"""# walk_{direction} QA

- Source strip: `assets/characters/purple_mage/walk/raw/walk_{direction}_6frame_staff_right_hand_green_strip.png`
- Alpha strip: `assets/characters/purple_mage/walk/raw/walk_{direction}_raw.png`
- Normalized strip: `assets/characters/purple_mage/walk/raw/walk_{direction}_normalized_strip.png`
- Normalized frames: `assets/characters/purple_mage/walk/frames/walk_{direction}_01.png` through `walk_{direction}_06.png`
- Preview: `assets/characters/purple_mage/walk/preview/walk_{direction}_preview.png`
- 4x preview: `assets/characters/purple_mage/walk/preview/walk_{direction}_preview_4x.png`
- Green 4x preview: `assets/characters/purple_mage/walk/preview/walk_{direction}_green_preview_4x.png`
- GIF: `assets/characters/purple_mage/walk/gif/walk_{direction}.gif`
- Mapped base source(s): `{base_sources}`
- Local tool: `assets/characters/purple_mage/walk/tools/walk_normalize_strip.py`

Checks performed:

- Six {direction}-facing frames exported at 128x128 RGBA.
...
def main() -> None:
    ...
    parser.add_argument("--walk-dir", default=Path(__file__).resolve().parents[1], type=Path)
    parser.add_argument("--base-sources", required=True)
    args = parser.parse_args()

    result = normalize_strip(args.source, args.direction, args.walk_dir)
    write_qa(args.direction, args.walk_dir, args.base_sources)
