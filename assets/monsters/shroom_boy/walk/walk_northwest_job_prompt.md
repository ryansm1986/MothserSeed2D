# Agent Job Prompt: walk_northwest

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_northwest`, which is `walk/northwest` with `8` frames.
...
1. Neutral planted waddle stance facing `northwest`; weight centered, hands slightly out for balance.
