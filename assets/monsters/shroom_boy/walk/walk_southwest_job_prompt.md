# Agent Job Prompt: walk_southwest

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_southwest`, which is `walk/southwest` with `8` frames.
...
1. Neutral planted waddle stance facing `southwest`; weight centered, hands slightly out for balance.
