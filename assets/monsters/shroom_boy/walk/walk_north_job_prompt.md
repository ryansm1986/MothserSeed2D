# Agent Job Prompt: walk_north

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_north`, which is `walk/north` with `8` frames.

Use these exact base picture reference(s): D:\projects\MotherSeed2D\assets\monsters\shroom_boy\base\north.png

Frame-by-frame pose plan:
1. Neutral planted waddle stance facing `north`; weight centered, hands slightly out for balance.
