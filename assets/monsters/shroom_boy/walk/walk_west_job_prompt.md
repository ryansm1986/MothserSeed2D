# Agent Job Prompt: walk_west

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_west`, which is `walk/west` with `8` frames.
...
1. Neutral planted waddle stance facing `west`; weight centered, hands slightly out for balance.
