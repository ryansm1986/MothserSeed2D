# Agent Job Prompt: walk_southeast

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_southeast`, which is `walk/southeast` with `8` frames.
...
1. Neutral planted waddle stance facing `southeast`; weight centered, hands slightly out for balance.
