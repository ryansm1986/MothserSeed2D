# Agent Job Prompt: walk_northeast

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_northeast`, which is `walk/northeast` with `8` frames.
...
1. Neutral planted waddle stance facing `northeast`; weight centered, hands slightly out for balance.
