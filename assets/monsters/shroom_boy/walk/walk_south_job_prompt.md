# Agent Job Prompt: walk_south

Use `$game-studio:sprite-pipeline`.

Task: Work only on `walk_south`, which is `walk/south` with `8` frames.
...
1. Neutral planted waddle stance facing `south`; weight centered, hands slightly out for balance.
