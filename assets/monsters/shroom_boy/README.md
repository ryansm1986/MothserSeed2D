...
- Each source-generation request must directly name the exact mapped base picture path or paths, such as `base/south.png` for cardinal directions or both adjacent base pictures for diagonals. Do not rely on a generic "use the base image" phrase.
