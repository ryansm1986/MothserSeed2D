...
- The source-generation prompt must directly reference the exact mapped base picture path or paths. For cardinal jobs, include the exact path such as `<BASE_FOLDER_PATH>/south.png`. For diagonal jobs, include both exact adjacent base picture paths such as `<BASE_FOLDER_PATH>/south.png` and `<BASE_FOLDER_PATH>/east.png`.
- The source-generation prompt must include a clear line like: `Use these exact base picture reference(s): <BASE_SOURCE_PATHS>`.
...
- Do not use a generic phrase like "the base image" by itself in the source-generation prompt. Name the actual mapped base picture path(s) so the image-generation request is anchored to the correct file(s).
