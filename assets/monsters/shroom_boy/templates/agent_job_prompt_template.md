...
- The source-generation request must directly reference the exact mapped base picture path(s): `<BASE_SOURCE_PATHS>`.
- The source-generation request must include this explicit line or equivalent wording: `Use these exact base picture reference(s): <BASE_SOURCE_PATHS>`.
- Do not say only "use the base image" or "reference the base" without naming the actual base picture path(s).
...
10. Generate one fresh one-direction source strip for `<JOB_ID>` using `$game-studio:sprite-pipeline`, and make sure the image-generation prompt directly names `<BASE_SOURCE_PATHS>` as the exact base picture reference(s).
