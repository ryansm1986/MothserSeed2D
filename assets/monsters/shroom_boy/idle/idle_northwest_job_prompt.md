# Agent Job Prompt: idle_northwest
...
Task: Work only on `idle_northwest`, which is `idle/northwest` with `5` frames.
...
1. Neutral `northwest` idle stance: feet/body planted, cap/head settled, face oriented correctly, hands relaxed near the body.
