...
Task: Work only on `idle_north`, which is `idle/north` with `5` frames.

Use these exact base picture reference(s): D:\projects\MotherSeed2D\assets\monsters\shroom_boy\base\north.png

Frame-by-frame pose plan:
1. Neutral `north` idle stance: feet/body planted, cap/head settled, face oriented correctly, hands relaxed near the body.
