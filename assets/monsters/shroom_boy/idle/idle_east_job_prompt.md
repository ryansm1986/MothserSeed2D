...
Task: Work only on `idle_east`, which is `idle/east` with `5` frames.
...
1. Neutral `east` idle stance: feet/body planted, cap/head settled, face oriented correctly, hands relaxed near the body.
