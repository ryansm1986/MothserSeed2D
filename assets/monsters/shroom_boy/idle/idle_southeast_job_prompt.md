# Agent Job Prompt: idle_southeast
...
Task: Work only on `idle_southeast`, which is `idle/southeast` with `5` frames.
...
1. Neutral `southeast` idle stance: feet/body planted, cap/head settled, face oriented correctly, hands relaxed near the body.
