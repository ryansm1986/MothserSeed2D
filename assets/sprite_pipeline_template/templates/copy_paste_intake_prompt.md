...
- Every source-generation prompt must directly reference the exact mapped base picture path or paths for the active job. For example, the prompt must include a line like `Use these exact base picture reference(s): <BASE_SOURCE_PATHS>`, with the actual `north.png`, `south.png`, `east.png`, or `west.png` path(s) filled in.
- Do not let a source-generation prompt say only "use the base image" or "reference the base" without naming the actual base picture path(s).
...
- Each source-generation request must include the exact mapped base picture path(s) from the active queue row.
