...
- For monsters, the initial source strip must be explicitly checked for at least one selected-frame width of empty space between neighboring visible pose bounds before cleanup, slicing, repack, or normalization.
...
- If full-frame spacing cannot fit inside the requested source image, request or build a wider horizontal source canvas. Increase canvas width, lane width, and flat `#ff00ff` gutters rather than reducing pose size.
...
- For monster jobs, treat the full-frame spacing rule as a hard gate. Measure the empty gutter between each neighboring pair of visible poses and confirm it is at least the selected monster frame width before accepting the strip.
- For monster action, spell, special, death, leap, wing, tail, or large-effect rows, use the expanded monster action profile spacing: `768px` for medium monsters and `1152px` for large monsters.
- Reject monster source strips where extended anatomy, weapons, shadows, glows, slashes, smoke, sparks, particles, or alpha haze touch another pose lane or reduce the empty gutter below one selected-frame width. Regenerate on a wider source canvas at the same monster scale.
...
- For monster jobs, the initial source strip was rejected and regenerated on a wider horizontal source canvas if any neighboring pose gap was less than one selected-frame width or if any effect/anatomy crossed into a neighboring pose lane.
